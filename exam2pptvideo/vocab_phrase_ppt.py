...
class VocabPhrasePPT:
  """It is designed as an abstract class to be inheritated for exam ppt generation of different languages,

  Note:
    **DON'T** use this class directly. To define a VocabPhrasePPT for a lanugage

      1. Define subclass inheriting ``VocabPhrasePPT``, e.g, ``ChineseVocabPhrasePPT`` 
  ...
  def __init__(self, sourcefile, title="", genre="classic"):
    ...
    if self.__class__.__name__ != "VocabPhrasePPT":
